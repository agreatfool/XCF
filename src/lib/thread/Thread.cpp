...
namespace XCF {
    ...
    Thread::Thread():
        threadId(NULL), lock(NULL), cond(NULL), status(ThreadStatus::INITED), logger(LogFactory::get())
    {
    ...
    pthread_mutex_t *ThreadUtil::createLock() {
        ...
        if (pthread_mutex_init(lock, NULL) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_mutex_init: [%d] %s", errno, strerror(errno)));
            return NULL;
        } else {
            return lock;
        }
    ...
    int32_t ThreadUtil::destroyLock(pthread_mutex_t *lock) {
        if (pthread_mutex_destroy(lock) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_mutex_destroy: [%d] %s", errno, strerror(errno)));
    ...
    int32_t ThreadUtil::lock(pthread_mutex_t *lock) {
        if (pthread_mutex_lock(lock) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_mutex_lock: [%d] %s", errno, strerror(errno)));
            return INVALID_RESULT;
        } else {
            return VALID_RESULT;
        }
    ...
    int32_t ThreadUtil::unlock(pthread_mutex_t *lock) {
        if (pthread_mutex_unlock(lock) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_mutex_unlock: [%d] %s", errno, strerror(errno)));
            return INVALID_RESULT;
        } else {
            return VALID_RESULT;
        }
    ...
    pthread_cond_t *ThreadUtil::createCond() {
        ...
        if (pthread_cond_init(cond, NULL) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_cond_inits: [%d] %s", errno, strerror(errno)));
            return NULL;
        } else {
            return cond;
        }
    ...
    int32_t ThreadUtil::destroyCond(pthread_cond_t *cond) {
        if (pthread_cond_destroy(cond) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_cond_destroy: [%d] %s", errno, strerror(errno)));
            return INVALID_RESULT;
        } else {
            delete cond;
            return VALID_RESULT;
    ...
    int32_t ThreadUtil::waitCond(pthread_cond_t *cond, pthread_mutex_t *lock) {
        if (pthread_cond_wait(cond, lock) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_cond_wait: [%d] %s", errno, strerror(errno)));
            return INVALID_RESULT;
        } else {
            return VALID_RESULT;
        }
    ...
    int32_t ThreadUtil::signalCond(pthread_cond_t *cond) {
        if (pthread_cond_signal(cond) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_cond_signal: [%d] %s", errno, strerror(errno)));
            return INVALID_RESULT;
        } else {
            return VALID_RESULT;
        }
    ...
    int32_t ThreadUtil::joinThread(pthread_t *threadId) {
        if (pthread_join(*threadId, NULL) < 0) {
            LogFactory::error(Utility::stringFormat("[Thread] error in pthread_join: [%d] %s", errno, strerror(errno)));
            return INVALID_RESULT;
        } else {
            return VALID_RESULT;
        }
